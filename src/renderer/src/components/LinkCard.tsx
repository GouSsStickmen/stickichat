...
export default function LinkCard(): React.JSX.Element | null {
  ...
  const hoverImagesOnly = useSettingsStore(
    (s) => s.settings.linkHoverImagesOnly,
  );
  ...
  return (
     ...
     *
     * `zoom` scales the element's own coordinate system, so putting it on the positioned element
     * scaled `left`/`top` along with everything else: at 150% a card meant for y=400 was drawn at
     * y=600, at 50% at y=200. That is the whole "the preview flies off up or down depending on
     * whether the size is above or below 100". The outer element carries the position in real
     * screen pixels and is never zoomed; the inner one carries the zoom. Measuring the outer one
     * still gives the on-screen size, because zoom changes layout and the wrapper sizes to it —
     * so the clamp keeps working in the units it thinks it is using.
     */
    <div
      ref={boxRef}
      className="link-card-anchor"
      style={{ left, top, visibility: box.h ? "visible" : "hidden" }}
    ...
    >
      <div
        ...
        style={
          scale !== 100
            ? ({ zoom: scale / 100 } as React.CSSProperties)
            : undefined
        }
      >
        {data.kind === "image" ? (
          ...
          <div className="lc-body" title={target.url} onClick={open}>
            ...
            <div className="lc-text">
              ...
              <div className="lc-title">
                {data.title ?? t("misc.linkShort")}
              </div>
              {data.description && (
                <div className="lc-desc">{data.description}</div>
              )}
            </div>
          </div>
        )}
